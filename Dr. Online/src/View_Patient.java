...
public class View_Patient extends JFrame {
	...
	public View_Patient() {
		...
		scrollPane.setBounds(5, 44, 630, 234);
		...
		lblNewLabel_1 = new JLabel("Patient Records");
		lblNewLabel_1.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel_1.setFont(new Font("Tahoma", Font.BOLD, 16));
		lblNewLabel_1.setBounds(5, 5, 636, 33);
		...
		btn_Close.setBounds(320, 283, 89, 23);
		contentPane.add(btn_Close);
		
		btn_ref = new JButton("Refresh");
		btn_ref.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				generate(x);
			}
		});
		btn_ref.setBounds(218, 283, 89, 23);
		contentPane.add(btn_ref);
		
		JLabel lblNewLabel = new JLabel("New label");
		lblNewLabel.setIcon(new ImageIcon("Images/Pat(big).jpg"));
	...
	private void generate(DBase x) {
		...
		try {
			...
			String sql1 = "select * from Records_Pat";
